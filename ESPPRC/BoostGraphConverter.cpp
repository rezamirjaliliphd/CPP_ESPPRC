// CPP file: BoostGraphConverter.cpp
...
//{
//    SPPRC_Example_Graph_Vert_Prop(int n = 0, double e = 0, double l = 0)
//        : num(n), eat(e), lat(l)
//    {
//    }
//    int num;
...
int rcBoost(const Graph& graph, const std::vector<double>& r_max, const Label& label)
{
    ...
    add_vertex(SPPRC_Example_Graph_Vert_Prop(label.vertex, 0,r_max[0]-label.resources[0]), g);
    add_vertex(SPPRC_Example_Graph_Vert_Prop(0, 0, 0), g);
	for (int i = 0; i < label.reachable.size(); i++) {
		if (label.reachable[i]) {
            add_vertex(SPPRC_Example_Graph_Vert_Prop(i, 0, 1000), g);
			std::cout << "Vertex " << i << " is added" << std::endl;
            for (const Edge& e : graph.getNeighbors(i,true))
            {
                if (label.reachable[e.to])
                {
                    add_edge(e.from, e.to, SPPRC_Example_Graph_Arc_Prop(cnt, e.cost, e.resources[0]), g);
					std::cout << "Edge " << e.from << " -> " << e.to << " is added. Cost " << e.cost  << std::endl;
                    cnt++;
                }
            }
    ...
    for (const Edge& e : graph.InList[label.vertex])
    {
        if (label.reachable[e.from])
        {
            add_edge(e.from, e.to, SPPRC_Example_Graph_Arc_Prop(cnt, e.cost, e.resources[0]), g);
			std::cout << "Edge " << e.from << " -> " << e.to << " is added. Cost "<< e.cost << std::endl;
            cnt++;
    ...
    for (const Edge& e : graph.OutList[0])
    {
        if (label.reachable[e.to])
        {
            add_edge(e.from, e.to, SPPRC_Example_Graph_Arc_Prop(cnt, e.cost, e.resources[0]), g);
	...
	std::cout << "Boost Graph is generated" << std::endl;
    ...
    graph_traits< SPPRC_Example_Graph >::vertex_descriptor s = 0;
    graph_traits< SPPRC_Example_Graph >::vertex_descriptor t = label.vertex;
    ...
    for (int i = 0; i < static_cast<int>(opt_solutions.size()); ++i)
    {
        for (int j = static_cast<int>(opt_solutions[i].size()) - 1; j >= 0;
            --j)
            std::cout << opt_solutions[i][j]<< std::endl;
    ...
    if (opt_solutions_spptw.size() > 0)
    {
        for (int i = 0; i < static_cast<int>(opt_solutions_spptw.size()); ++i)
        {
            for (int j = static_cast<int>(opt_solutions_spptw[i].size()) - 1;
                j >= 0; --j)
                std::cout << opt_solutions_spptw[i][j] << std::endl;
        ...
        for (int i = 0; i < static_cast<int>(opt_solutions_spptw.size()); ++i)
        {
            ...
            for (int j = static_cast<int>(opt_solutions_spptw[i].size()) - 1;
                j >= 0; --j)
                std::cout << source(opt_solutions_spptw[i][j], g)
                << std::endl;
